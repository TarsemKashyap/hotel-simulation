...
export class AccountService {
  ...
  constructor(private httpClient: HttpClient, private sessionStore: SessionStore) { }
  ...
  login(login: LoginModel): Observable<LoginResponse> {
    return this.httpClient.post<LoginResponse>("account/login", login).pipe(map(x => {
  ...
  refreshToken():Observable<boolean> {
   ...
   return this.httpClient.post<any>("account/token/refresh", credentials).pipe(map(x => {
      this.sessionStore.SetAccessToken(x.accessToken);
      this.sessionStore.SetRefreshToken(x.refreshToken);
      return true;
    }));
