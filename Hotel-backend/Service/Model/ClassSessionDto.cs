...
public class ClassSessionDtoValidator : AbstractValidator<ClassSessionDto>
{
    public ClassSessionDtoValidator()
    {
        ...
        RuleFor(x => x.StartDate).NotNull().NotEmpty().WithMessage("Start date should not be empty or null").Must(x => ValidateData(x)).WithMessage("Start date is not valid");
        RuleFor(x => x.EndDate).NotNull().NotEmpty().WithMessage("Start date should not be empty or null").Must(x => ValidateData(x)).WithMessage("End date is not valid");
